...
test.describe("Browser Research", () => {
  test("research Android adaptive icon documentation", async ({ page }) => {
    ...
    await page.goto(
      "https://developer.android.com/develop/ui/views/launcher/icon-design"
    );
    ...
    const hasSafeZone =
      safeZoneText?.includes("safe zone") || safeZoneText?.includes("72dp");

    console.log("\nContains safe zone information:", hasSafeZone);

    // Take a screenshot for reference
    await page.screenshot({
      path: "tests/screenshots/icon-docs.png",
      fullPage: true,
    });
  ...
  test("research Compose BOM versions", async ({ page }) => {
    ...
    const latestVersion = versionInfo?.match(
      /202[0-9]\.[0-9]{2}\.[0-9]{2}/
    )?.[0];
    ...
    await page.screenshot({
      path: "tests/screenshots/bom-mapping.png",
      fullPage: true,
    });
